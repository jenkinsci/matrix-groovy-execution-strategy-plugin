...
class DynamicGroovyMES extends MatrixExecutionStrategy {
    ...
    public Result run(MatrixBuild.MatrixBuildExecution execution) throws InterruptedException, IOException {
        ...
        combs.each {
            scheduleConfigurationBuild(execution, mc[it])
        }

        Result r = Result.SUCCESS

        combs.each {
            MatrixRun run = waitForCompletion(execution, mc[it]);
            notifyEndBuild(run,execution.getAggregators());
            execution.getListener().getLogger().println('Completed ' + ModelHyperlinkNote.encodeTo(mc[it]) + ' '  + getResult(run))
